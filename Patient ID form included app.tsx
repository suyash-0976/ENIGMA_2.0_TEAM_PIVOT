...
import { Upload, Activity, Brain, AlertCircle, CheckCircle2, FileText, User } from 'lucide-react';
...
function App() {
  ...
  // Naye Patient Details wale variables
          ...
          {result && (
            <div className={p-6 rounded-2xl border ${result.risk_score > 50 ? 'bg-red-950/30 border-red-900' : 'bg-green-950/30 border-green-900'}}>
              <div className="mb-4 pb-4 border-b border-slate-800/50">
                 <h3 className="text-lg font-semibold text-white mb-2">Clinical Report Summary</h3>
                 <div className="grid grid-cols-2 gap-2 text-sm text-slate-300">
                   <div><span className="text-slate-500">Name:</span> {patientName || 'N/A'}</div>
                   <div><span className="text-slate-500">ID:</span> {patientId}</div>
                   <div><span className="text-slate-500">Age:</span> {patientAge ? ${patientAge} yrs : 'N/A'}</div>
                   <div><span className="text-slate-500">Gender:</span> {patientGender || 'N/A'}</div>
                 </div>
              </div>
